...
const CurrentMonthWorklogHour = ({ navigation }: any) => {
    const Assesstoken = useSelector((state: any) => state?.appState?.authToken);
    const accessToken = Assesstoken?.authToken?.accessToken;
    const { data: isloggeddata, isLoading: islogged } = useGetWorkLogThisMonthAndLastMonthQuery({ accessToken, month: 'thismonth' })
    const { data: isApproveddata, isLoading: isApproved } = useApprovedWorkLogLast30DaysQuery({ accessToken, month: 'thismonth' })
...
export default CurrentMonthWorklogHour